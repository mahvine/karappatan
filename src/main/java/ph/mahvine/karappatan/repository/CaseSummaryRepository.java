...
public interface CaseSummaryRepository extends JpaRepository<CaseSummary, Long> {
    ...
    @Query("select distinct case_summary from CaseSummary case_summary left join fetch case_summary.answers where case_summary.user.login = ?#{principal.username} ")
